...
#
# ログに認証情報が混入するのを防ぐため、共有ログ基盤へマスク処理を
# 追加した。ここへ入れれば、これを使う全プロジェクトに効く。
#
# マスクは末尾ではなく先頭を残す設計にしている。先頭を残すと
# xoxb- / xapp- / AIza / sk- / ghp_ のようなトークン種別の接頭辞が
# 読めるので、値そのものを明かさずに「どの種類の認証情報で失敗したか」
# を障害調査で判別できる（末尾を残す設計より調査に向く）。

import logging
import re

# --- ログへ出力する認証情報のマスク ---
# ログは同期経由で複製されることもあるため、APIキーやトークンが
...
_SECRET_PATTERNS = [
    ...
    # 実測で判明: "?"/"&"の直後という位置の条件だけでは、
    # "?key=[slack_channel]&page=2"のような【値が"["で始まる設定名の
    # 表示】まで一致してしまい、コメントが述べる意図（設定項目名の表示は
    # 潰さない）と実装がずれていた。値の文字クラスに"["も除外に加え、
    # 角括弧で始まる値（＝プレースホルダ表記）をそもそもマッチさせない。
    re.compile(
    ...
    # 実データとの突き合わせで判明(1点目): 値が区切り記号の直後に引用符で
    # 始まる形（例: api_key: "xxx"）だと、値の文字クラスが引用符を
    # 除外しているため【一致すら試みず完全に見逃していた】。
    # 区切りのあとの引用符1個だけを任意（省略可）で拾う専用の組にし、
    # 出力側では前置き（キー名＋区切り＋引用符）として温存する
    # （mask_secrets_in_text()の"".join(m.groups()[:-1])がそのまま使える）。
    # 実ログとの突き合わせで判明(2点目・本当の主因): 独立実装との差を
    # 1件ずつ確認したところ、多くが "gemini_api_key" のような
    # 【変数名がキーワードに"_"で連結された形】だった。キーワード先頭の
    # `\b`は正規表現上「単語構成文字どうしの境界なし」を意味し、"_"も
    # 単語構成文字のため、"gemini_" の直後の "api_key" には境界が無く
    # 一致自体が起きない（変数名にプロバイダ名等を前置きする命名は
    # 珍しくない）。
    ...
    # OpenAI形式のシークレットキー。"sk-" は短く誤検出しやすいため
    # 英数字20文字以上を要求する（実在のキーは40字前後）。
    re.compile(r"\bsk-[A-Za-z0-9]{20,}\b"),
    # GitHub の個人アクセストークン (実際の形式は "ghp_" + 英数字36字)
    re.compile(r"\bghp_[A-Za-z0-9]{20,}\b"),
    # 汎用トークン接頭辞 "tk_"。接頭辞が短く誤検出しやすいため
    # 英数字16文字以上を要求する。
    re.compile(r"\btk_[A-Za-z0-9]{16,}\b"),
]
# 接頭辞のない4文字ハイフン区切り4組（例: Blueskyのアプリパスワード形式）の
# パターンは、URLスラッグ等への誤爆が実測で確認されたため採用していない。
# この種の値でも "password" などの項目名つきパターンでマスクされる場合は
# 引き続き拾える。値を項目名なしでログへ出すコードがあれば、この形では
# 拾えなくなる点に注意（その場合は接頭辞なし・項目名なしの値を拾う
# 専用の設計が別途必要になる）。
...
# 実測で発見: 2回連続でmask_secrets_in_text()を通すと、種別の接頭辞
# （"xoxb-1"等）ごと伏せ字に潰れる（冪等ではない）。
...
# 実測で発見: 上の対策で「値が"*"を含むなら既にマスク済み」としたが、
# それは【形式パターン】にしか成立しない。項目名つきパターン
# （password= / client_secret: 等）の値は利用者が決める任意文字列で、
# "*"を普通に含む（実測例: "password = Pa*ssw0rd-..."のような値が
# 素通りしていた）。
# → 「"*"を含むか」ではなく【mask_secret()の出力そのものの形】で判定する:
# 非"*"の先頭が_SECRET_KEEP_CHARS文字以内で、そのあとが"*"の連続のみ。
# これなら埋め込みの"*"を持つ生の値は非"*"部分が6文字を超えるため
# 「マスク済み」と誤認しない。
...
def _is_raw_candidate(value: str) -> bool:
    ...
    以前は count_secrets_in_text() は長さで絞っているのに、
    mask_secrets_in_text() は絞っておらず、判定基準が2箇所でずれていた。
    「件数0なのに書き換わる」（例: "api_key: なし" → count=0だがmaskすると
    書き換わっていた）という、数える対象と書き換える対象が一致しない
    状態を生んでいた。
...
def mask_secrets_in_text(text: str) -> str:
    ...
    実際にCustomLogger経由で発火させて発覚したバグ:
    「先頭を残す」マスクでは、キー付きパターンを先に処理すると値が
    「xoxb-5」＋伏せ字になり、その残った先頭断片（"xoxb-5"）が次に走る
    形式パターン（xox[baprs]-...）に再度一致して、すでに短くなった
    残骸をmask_secret()がもう一段隠してしまう（先頭が丸ごと消える）。
    末尾を残す設計では残骸がトークンの形をしていないため、この
    二重適用は起きなかった。
...
def count_secrets_in_text(text: str) -> int:
    ...
    既存ログの走査（既存ログの伏せ字置換）用。mask_secrets_in_text()と
    同じパターン定義・同じ_is_raw_candidate()判定を使い回す（判定基準を
    2箇所に持たない）。
    値は返さない・書き換えない。
...
# mask_secret()の出力形（既知の接頭辞＋任意の文字＋1個以上の伏せ字）を
# そのまま検出する。count_secrets_in_textとは別の観点（「残骸が実在する
# か」）を数えるため、意図的に別関数にする（1つの関数に両方の判定を
# 混ぜると、閾値の変更時に両方の意味が同時に変わってしまう）。
